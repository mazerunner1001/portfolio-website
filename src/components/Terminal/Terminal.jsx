...
const Terminal = ({ isOpen, onClose, onMinimize, onMaximize, isMaximized, isMinimized, minimizedPosition, onMinimizedDrag, previousWindowState, onWindowStateUpdate }) => {
  ...
  useEffect(() => {
    const handleMinimizedMouseMove = (e) => {
      if (isBubbleDragging) {
        ...
        // Calculate drag distance to determine if this is a drag vs click
        ...
        const bubbleSize = 80; // Bubble width/height
        const newX = Math.max(0, Math.min(window.innerWidth - bubbleSize, e.clientX - dragOffset.x));
        const newY = Math.max(0, Math.min(window.innerHeight - bubbleSize, e.clientY - dragOffset.y));
  ...
  }, [isBubbleDragging, dragOffset, onMinimizedDrag, initialMousePos]);

  // Update parent component when window state changes (debounced)
  useEffect(() => {
    if (onWindowStateUpdate && !isDragging && !isResizing) {
  ...
  }, [isMaximized, windowPosition, windowSize, onWindowStateUpdate, isDragging, isResizing]);
  ...
  const handleMinimizedMouseDown = (e) => {
    e.preventDefault();
    e.stopPropagation();
    if (isMinimized && minimizedRef.current) {
  ...
  const handleBubbleClick = (e) => {
    ...
    // Only trigger maximize if drag distance is minimal (< 5 pixels)
    if (dragDistance < 5) {
      setTimeout(() => {
        onMaximize();
        setDragDistance(0);
  ...
  if (isMinimized) {
    return (
      <div 
        ref={minimizedRef}
        className={`terminal-minimized-bubble ${isBubbleDragging ? 'dragging' : ''}`}
        style={{
          position: 'fixed',
      ...
      >
        <div className="bubble-close-btn" onClick={handleBubbleClose}>
          ×
        </div>
